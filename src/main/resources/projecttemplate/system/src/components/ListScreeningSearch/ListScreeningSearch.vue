<template>
  <div class="list-screening-search">
    <el-form label-width="100px" :model="searchData">
      <div v-for="item in listLine" :key="item.id" class="list-margin">
        ...
        <div class="line-content">
          ...
          <div v-for="(dict, index) in item.isDictionaries ? $store.getters.getDicts(item.id,true) : item.itemList" :key="index" class="label-radio unlimited-style" :class="{'active': searchData[item.id] === dict.dictCode}">
            <el-tooltip v-if="dict.dictState && dict.dictState === 3" content="该员工已离职" placement="top" effect="light">
              <span style="text-decoration:line-through;text-decoration-color: red;" @click="selectData(item.id, dict.dictionaryCode)">{{ dict.dictionaryName }}</span>
            </el-tooltip>
            <span v-else @click="selectData(item.id, dict.dictCode)">{{ dict.dictName }}</span>
          </div>
        </div>
      </div>
      <div v-if="isShopCategory" class="list-margin">
        <div class="line-name">网店类目</div>
        <div class="line-content">
          <div class="label-radio unlimited-style" :class="{'active': oneLevelCategory.length === 0}" style="float: left">
            <span @click="clearShopCategory('shopCategory')">不限</span>
          </div>
          <div class="item-shopCategory">
            <div v-for="item in searchData.shopType === '' || searchData.shopType !== 3 ? TianMaoCategory : JDCategory" :key="item.id" class="label-radio" :class="{'active': oneLevelCategory === item.child}">
              <span @click="selectShopCategory(item)">{{ item.name }}</span>
            </div>
          </div>
        </div>
      </div>
      <div v-if="isShopCategory && oneLevelCategory.length > 0" class="list-margin">
        <div class="line-name">一级类目</div>
        <div class="line-content">
          <div class="label-radio unlimited-style" style="float: left" :class="{'active': publicShopCategory.length === 0}">
            <span @click="clearShopCategory('oneLevelCategory')">不限</span>
          </div>
          <div class="item-shopCategory">
            <el-checkbox-group v-model="publicShopCategory" class="label-radio">
              <el-checkbox v-for="item in oneLevelCategory" :key="item.id" :label="item">
                {{ item.name }}
              </el-checkbox>
            </el-checkbox-group>
          </div>
      ...
      <div v-if="isShopCategory && publicShopCategory.length > 0" class="list-margin">
        <div class="line-name">已选类目</div>
        <div class="line-content">
          <div class="unlimited-style">
            <template v-for="(item, index) in publicShopCategory">
              <el-tag v-if="item.id" :key="item.id" closable disable-transitions type="warning" @close="removeDomain(index)">
                {{ item.name }}
              </el-tag>
            </template>
          </div>
      ...
      <div v-if="isShopTrademarkClassify" class="list-margin">
        <div class="line-name">商标类别</div>
        <div class="line-content">
          <div class="label-radio unlimited-style" :class="{'active': publicShopTrademarkClassify.length === 0}" style="float: left">
            <span @click="clearShopTrademarkClassify">不限</span>
          </div>
          <div class="item-shopCategory">
            <el-checkbox-group v-model="publicShopTrademarkClassify" class="label-radio">
              <el-checkbox v-for="item in shopTrademarkClassify" :key="item.id" :value="item.id" :label="item.id">
                {{ '第' + (item.num ? item.num : '') + '类' }}
              </el-checkbox>
            </el-checkbox-group>
          </div>
        </div>
        ...
        <div v-for="(item, index) in listLineItem" :key="index" class="label-input">
          ...
          <template v-if="item.type === 'select'">
            <el-select v-model="searchData[item.id]" size="mini" clearable :placeholder="item.name" @clear="clearData" @keyup.enter.native="queryTableData">
              <el-option v-for="select in item.itemList" :key="select.name" :label="select.name" :value="select.name"></el-option>
            </el-select>
          </template>
...
<script>
import shopTrademarkClassify from '@/assets/js/tradType'
import TianMaoCategory from '@/assets/js/shops'
import JDCategory from '@/assets/js/JDCategory'
export default {
  ...
  data() {
    return {
      ...
      listLine: [],
      listLineItem: [],
      TianMaoCategory: TianMaoCategory,
      JDCategory: JDCategory,
      shopTrademarkClassify: shopTrademarkClassify,
      shopCategories: [],
      oneLevelCategory: [],
      publicShopCategory: [],
  ...
  watch: {
    publicShopCategory: {
      handler(newVal) {
        this.searchData[this.shopCategoryName] = []
        newVal.forEach(item => {
          this.searchData[this.shopCategoryName].push({
            shopCategory: item.id
          })
        })
        this.queryTableData()
      }
    ...
    publicShopTrademarkClassify: {
      handler(newVal) {
        this.searchData[this.shopTrademarkClassifyName] = []
        newVal.forEach(item => {
          this.searchData[this.shopTrademarkClassifyName].push({
            shopTrademarkClassify: item
          })
        })
        this.queryTableData()
      }
  ...
  methods: {
    ...
    // 清空选择的网店类目
    clearShopCategory(value) {
      if (value === 'shopCategory') {
        this.oneLevelCategory = []
      }
      this.publicShopCategory = []
    },
    // 商标类别选择不限
    clearShopTrademarkClassify() {
      this.publicShopTrademarkClassify = []
    },
    // 选择时间排序
