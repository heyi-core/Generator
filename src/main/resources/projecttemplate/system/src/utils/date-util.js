import fecha from 'heyi-ui/lib/utils/date'
import { t } from 'heyi-ui/lib/locale'
...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`el.datepicker.weeks.${r'${'}week}`)),
    dayNames: weeks.map(week => t(`el.datepicker.weeks.${r'${'}week}`)),
    monthNamesShort: months.map(month => t(`el.datepicker.months.${r'${'}month}`)),
    monthNames: months.map((month, index) => t(`el.datepicker.month${r'${'}index + 1}`)),
    amPm: ['am', 'pm']
  }
}
